...
Let's update **{module}** folder from Chapter 3.1, by adding another script called `matrix_ops.r` to store functions that rewrites `*` for matrix multiplication and overrides `^` where a specific value, `^-1`, implies the inverse of the matrix, and `tables.r` to store functions for displaying tables in R REPL, and extending the **{statistics}** subfolder by adding another level of organization, e.g. you want to add statistical model utilities organized by model type, saved as **{models}** subfolder.
...
In this structure, **{models}** is nested two levels deep from the root **{module}** folder. Each directory still requires its own `__init__.r` file to be recognized as a module.
...
Under **{models}** subfolder, place this in `__init__.r` file:
...
And since we are adding another module within **{statistics}**, update its `__init__.r` initialization file:
...
-   **Organize by functionality**: Group related functions in a way that makes sense for your workflow. For example, keeping all model-related functions under **{models}**, organized by algorithm type.
